...
try:
    ...
    if tkinter.messagebox.askyesno("Error", "MySQL is not installed\nYou need to install MySQL\nWould you like to run MySQL installer?"):
        try:
            os.system(r".\additional\mysql.msi")
        except:
...
def check_table(*args, **kwargs):
    ...
    if tables==ideal:
        ...
        return False
with open(r".\additional\default_values.txt") as f:
    ah= f.read().split(",")
    pre_user= ah[0]
    pre_pwd=ah[1]
    pre_db=ah[2]
def main(entry, *args, **kwargs):
    title= Label(entry, text="Welcome to Store Manager by ANSH PANT", font=("Stylish",21, "bold"), bg="beige", fg="#EE8262")
    ...
    sign_img= PhotoImage(file=".\\additional\\sign.png")
    entry.one = sign_img
    ...
    def fun_default(*args, **kwargs):
        ...
        def dooit(*args, **kwargs):
            if tkinter.messagebox.showinfo("Confirmation", "Are you sure you want to\nchange the default valeus"):
                with open(r".\additional\default_values.txt", "w+") as f:
    ...
    def else_load(*args, **kwargs):
        def create_fun(*args, **kwargs):
            ...
            entry.title("STORE MANAGEMENT.create_database")
            db_e.delete(0, END)
            db_e.focus()
        ...
        def load_fun(*args, **kwargs):
            ...
            entry.title("STORE MANAGEMENT.load_database")
            deff.destroy()
            fr= Frame(entry, height=620, width=550, bg="beige")
            fr.grid(row=0, column=0)
            title= Label(fr, text="Welcome to Store Manager by ANSH PANT", font=("Comic Sans", 22, "bold"), bg="beige", fg="#EE8262")
            title.place(x=30, y=10)
        ...
        deff.wm_iconbitmap(r".\additional\icon.ico")  
...
entry=Tk()
entry.title("STORE MANAGEMENT.enter")
entry.wm_iconbitmap(r".\additional\icon.ico")
entry.geometry("550x500+160+15")
...
def stock_addition(*args, **kwargs):
    try:
        left.pack_forget()
    except:
        pass
    ...
    class Stock_addition:
        def __init__(self, root,*args, **kwargs):
            root1.title("STORE MANAGEMENT.Addition")
            try:
            ...
            def back(*args, **kwargs):
                root1.title("STORE MANAGEMENT.Billing")
...
def stock_modify(*args, **kwargs):
    ...
    class Stock_modify:
        def __init__(self, root,*args, **kwargs):
            root1.title("STORE MANAGEMENT.Modification")
            self.conn= sql.connect(host="localhost", user=uname, passwd=pwd, database=dbName)
            canvas= Frame(root, width=765, height=600, bg="#88BDBC")
...
def stock_review(*args, **kwargs):
    class Stock_review:
        def __init__(self, root,*args, **kwargs):
            root1.title("STORE MANAGEMENT.Stock_Review")
            self.conn= sql.connect(host="localhost", user=uname, passwd=pwd, database=dbName)
...
def sold_item_review(*args, **kwargs):
    class Sold_item_review: 
        def __init__(self, root,*args, **kwargs):
            root1.title("STORE MANAGEMENT.Sold_Item_Review")
            self.conn= sql.connect(host="localhost", user=uname, passwd=pwd, database=dbName)
            ...
            def show_bill(*args, **kwargs):
                ...
                if selc!=():
                    ...
                    dr=f".\\additional\\Invoices\\{invoicedate}\\{invoiceid}.rtf"
...
class Billing:
    def __init__(self,root1,left, right, *args, **kwargs):
        ...
        def on_closing2(*args, **kwargs): 
            if tkinter.messagebox.askokcancel("Close Tabs", "Are you sure to close the main window?"):
                closer()
                sys.exit(0)
        root1.protocol("WM_DELETE_WINDOW", on_closing2)
        self.uptstock=0
    ...
    def gbill(self, *args,**kwargs):
        if len(prodquan)==0:
            ...
            def submit_fun(*args, **kwargs):
                ...
                dr="./additional/Invoices/" + str(date)
                ...
                if not os.path.exists(dr):
                    os.makedirs(dr)
                f= open(r".\additional\inid.txt",'r+')
                randomid= f.read()
                f.truncate(0)
                f.seek(0)
            ...
            def topp(sel, *args, **kwargs):
                ...
                if sel=="By Percentage":
                    def back1(*args, **kwargs):
                        ...
                        top_l1.place(x=10, y=10)
                    self.img_bill= PhotoImage(file=".\\additional\\backkk.png")
...
root1.title("STORE MANAGEMENT.Billing")
root1.wm_iconbitmap(r".\additional\icon.ico") 
root1.focus()
